  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        getEnrollInstanceItemDetail({ 'session': document.cookie, 'itemId': this.itemId }).then(res => {
          this.item = res.data
        }).catch(err => {
      ...
      submit(){
        saveOrUpdateEnrollInstanceItem({'session': document.cookie,'item':this.item}).then(res => {
          if (res.re === 1) {
            this.$message({
              message: '保存成功',
              type: 'sucess'
